...
$studentSql = mysql_query("Select * from tbl_amenities");
$i = 0;
$recruitementArray = array();
while ($row = mysql_fetch_assoc($studentSql)) {
    $recruitementArray[$i]['amenities_name'] = $row['amenities_name'];
    $i++;
}
...
<body>
    ...
    <?php if (count($recruitementArray) > 0) { ?>
        <table id="example" class="table table-striped" cellspacing="0" width="100%">
            ...
            ?>
            <tr>
                <td><?php echo $recruitementArray[$i]['amenities_name']; ?></td>
            </tr>
            <?php } ?>
